import React, { useState } from 'react';
import { 
  FileText, 
  Calendar, 
  MapPin, 
  Scale, 
  ExternalLink, 
  Eye, 
  Download,
  BookOpen,
  Gavel,
  Star,
  Users,
  ChevronRight,
  Tag
} from 'lucide-react';
import type {
  SearchResult, 
  JurisprudenceResult, 
  LegalText,
  PrecedentValue 
} from '../../types/search';
import { Language } from '../../types';

interface SearchResultsProps {
  results: SearchResult<JurisprudenceResult | LegalText>;
  searchType: 'jurisprudence' | 'legal_texts';
  language: Language;
  theme?: 'light' | 'dark';
  onResultClick?: (result: JurisprudenceResult | LegalText) => void;
}

const SearchResults: React.FC<SearchResultsProps> = ({
  results,
  searchType,
  language,
  theme = 'light',
  onResultClick
}) => {
  const isAr = language === 'ar';
  const [expandedResults, setExpandedResults] = useState<Set<string>>(new Set());

  const toggleExpanded = (id: string) => {
    const newExpanded = new Set(expandedResults);
    if (newExpanded.has(id)) newExpanded.delete(id);
    else newExpanded.add(id);
    setExpandedResults(newExpanded);
  };

  const getPrecedentValueColor = (value: PrecedentValue) => {
    switch (value) {
      case 'binding': return 'bg-red-100 text-red-800 dark:bg-red-900/20 dark:text-red-300';
      case 'persuasive': return 'bg-amber-100 text-amber-800 dark:bg-amber-900/20 dark:text-amber-300';
      case 'informative': return 'bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-300';
      default: return 'bg-slate-100 text-slate-800 dark:bg-slate-800 dark:text-slate-300';
    }
  };

  const getPrecedentValueLabel = (value: PrecedentValue) => {
    const labels: Record<PrecedentValue, string> = {
      binding: isAr ? 'ملزم' : 'Contraignant',
      persuasive: isAr ? 'مقنع' : 'Persuasif',
      informative: isAr ? 'إعلامي' : 'Informatif',
    };
    return labels[value] ?? value;
  };

  const formatDate = (dateStr: string) => {
    return new Date(dateStr).toLocaleDateString(isAr ? 'ar-DZ' : 'fr-FR', {
      year: 'numeric', month: 'long', day: 'numeric'
    });
  };

  const highlightSearchTerms = (text: string, searchQuery: string) => {
    if (!searchQuery) return text;
    const terms = searchQuery.split(' ').filter(t => t.length > 2);
    let out = text;
    terms.forEach(term => {
      const regex = new RegExp(`(${term})`, 'gi');
      out = out.replace(regex, '<mark class="bg-yellow-200 dark:bg-yellow-800 px-1 rounded">$1</mark>');
    });
    return out;
  };

  if (!results || results.items.length === 0) {
    return (
      <div className="text-center py-12">
        <div className="mb-4">
          {searchType === 'jurisprudence'
            ? <Gavel size={48} className="mx-auto text-slate-300" />
            : <BookOpen size={48} className="mx-auto text-slate-300" />}
        </div>
        <h3 className="text-lg font-medium text-slate-500 mb-2">
          {isAr ? 'لا توجد نتائج' : 'Aucun résultat trouvé'}
        </h3>
        <p className="text-slate-400 text-sm">
          {isAr ? 'جرب مصطلحات بحث مختلفة أو قم بتعديل المرشحات' : 'Essayez des termes différents ou modifiez les filtres'}
        </p>
      </div>
    );
  }

  return (
    <div className="space-y-6" dir={isAr ? 'rtl' : 'ltr'}>
      {results.items.map((result) => {
        const isExpanded = expandedResults.has(result.id);
        const isJurisprudence = searchType === 'jurisprudence';
        const j = result as JurisprudenceResult;
        const lt = result as LegalText;
        const displayTitle = isJurisprudence ? (j.summary?.substring(0, 80) + '…') : lt.title;

        return (
          <div
            key={result.id}
            className={`border rounded-2xl overflow-hidden transition-all duration-200 hover:shadow-lg ${
              theme === 'dark'
                ? 'bg-slate-800 border-slate-700 hover:border-slate-600'
                : 'bg-white border-slate-200 hover:border-slate-300'
            }`}
          >
            <div className="p-6">
              <div className="flex items-start justify-between mb-4">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    {isJurisprudence
                      ? <Gavel className="text-legal-blue flex-shrink-0" size={20} />
                      : <BookOpen className="text-legal-blue flex-shrink-0" size={20} />}

                    <span className="text-sm font-mono text-slate-500 bg-slate-100 dark:bg-slate-700 px-2 py-1 rounded">
                      {isJurisprudence ? j.caseNumber : lt.reference}
                    </span>

                    {isJurisprudence && j.precedentValue && (
                      <span className={`text-xs font-bold px-2 py-1 rounded-full ${getPrecedentValueColor(j.precedentValue)}`}>
                        {getPrecedentValueLabel(j.precedentValue)}
                      </span>
                    )}

                    {isJurisprudence && j.relevanceScore != null && (
                      <div className="flex items-center gap-1">
                        <Star size={14} className="text-amber-500" />
                        <span className="text-xs text-slate-500">{Math.round(j.relevanceScore * 100)}%</span>
                      </div>
                    )}
                  </div>

                  <h3
                    className="text-lg font-bold text-slate-900 dark:text-slate-100 mb-3 cursor-pointer hover:text-legal-blue transition-colors"
                    onClick={() => onResultClick?.(result)}
                    dangerouslySetInnerHTML={{ __html: highlightSearchTerms(displayTitle, results.query.text) }}
                  />

                  <div className="flex flex-wrap items-center gap-4 text-sm text-slate-500 mb-3">
                    <div className="flex items-center gap-1">
                      <Calendar size={14} />
                      {isJurisprudence ? formatDate(j.date) : formatDate(lt.publicationDate)}
                    </div>
                    {isJurisprudence && (
                      <div className="flex items-center gap-1">
                        <MapPin size={14} />
                        {j.court.name}
                      </div>
                    )}
                    <div className="flex items-center gap-1">
                      <Scale size={14} />
                      {isJurisprudence ? j.legalDomain : lt.domain}
                    </div>
                    {isJurisprudence && j.parties && j.parties.length > 0 && (
                      <div className="flex items-center gap-1">
                        <Users size={14} />
                        {j.parties.slice(0, 2).map(p => p.name).join(' vs ')}
                        {j.parties.length > 2 && '…'}
                      </div>
                    )}
                  </div>

                  <div className="text-slate-600 dark:text-slate-300 text-sm leading-relaxed">
                    {isJurisprudence ? (
                      <p dangerouslySetInnerHTML={{ __html: highlightSearchTerms(j.summary ?? '', results.query.text) }} />
                    ) : (
                      <p dangerouslySetInnerHTML={{ __html: highlightSearchTerms(lt.content.substring(0, 300) + '…', results.query.text) }} />
                    )}
                  </div>

                  {isJurisprudence && j.keywords && j.keywords.length > 0 && (
                    <div className="mt-3 flex flex-wrap gap-2">
                      {j.keywords.slice(0, 5).map((kw, i) => (
                        <span key={i} className="inline-flex items-center gap-1 px-2 py-1 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 text-xs rounded-full">
                          <Tag size={10} />
                          {kw}
                        </span>
                      ))}
                      {j.keywords.length > 5 && (
                        <span className="text-xs text-slate-400">+{j.keywords.length - 5} {isAr ? 'أخرى' : 'autres'}</span>
                      )}
                    </div>
                  )}
                </div>

                <div className="flex items-center gap-2 ml-4">
                  <button
                    onClick={() => toggleExpanded(result.id)}
                    className="p-2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 transition-colors"
                    title={isExpanded ? (isAr ? 'إخفاء' : 'Réduire') : (isAr ? 'توسيع' : 'Développer')}
                  >
                    <Eye size={16} />
                  </button>
                  {isJurisprudence && j.documentUrl && (
                    <button
                      onClick={() => window.open(j.documentUrl, '_blank')}
                      className="p-2 text-slate-400 hover:text-legal-blue transition-colors"
                      title={isAr ? 'فتح المستند' : 'Ouvrir le document'}
                    >
                      <ExternalLink size={16} />
                    </button>
                  )}
                  <button className="p-2 text-slate-400 hover:text-legal-blue transition-colors" title={isAr ? 'تحميل' : 'Télécharger'}>
                    <Download size={16} />
                  </button>
                </div>
              </div>

              {isExpanded && (
                <div className={`mt-6 pt-6 border-t ${theme === 'dark' ? 'border-slate-700' : 'border-slate-200'}`}>
                  {isJurisprudence ? (
                    <div className="space-y-4">
                      {j.fullText && (
                        <div>
                          <h4 className="font-bold text-sm text-slate-700 dark:text-slate-300 mb-2">
                            {isAr ? 'النص الكامل' : 'Texte intégral'}
                          </h4>
                          <div
                            className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed max-h-64 overflow-y-auto"
                            dangerouslySetInnerHTML={{ __html: highlightSearchTerms(j.fullText, results.query.text) }}
                          />
                        </div>
                      )}
                      {j.citations && j.citations.length > 0 && (
                        <div>
                          <h4 className="font-bold text-sm text-slate-700 dark:text-slate-300 mb-2">
                            {isAr ? 'المراجع القانونية' : 'Références légales'}
                          </h4>
                          <div className="space-y-1">
                            {j.citations.map((c, i) => (
                              <div key={i} className="text-sm text-slate-600 dark:text-slate-400">
                                <span className="font-medium">{c.reference}</span>
                                {c.text && <span className="ml-2">— {c.text}</span>}
                              </div>
                            ))}
                          </div>
                        </div>
                      )}
                    </div>
                  ) : (
                    <div className="space-y-4">
                      <div>
                        <h4 className="font-bold text-sm text-slate-700 dark:text-slate-300 mb-2">
                          {isAr ? 'النص الكامل' : 'Texte complet'}
                        </h4>
                        <div
                          className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed max-h-64 overflow-y-auto"
                          dangerouslySetInnerHTML={{ __html: highlightSearchTerms(lt.content, results.query.text) }}
                        />
                      </div>
                      {lt.joraReference && (
                        <div>
                          <h4 className="font-bold text-sm text-slate-700 dark:text-slate-300 mb-2">
                            {isAr ? 'مرجع الجريدة الرسمية' : 'Référence JORA'}
                          </h4>
                          <p className="text-sm text-slate-600 dark:text-slate-400">{lt.joraReference}</p>
                        </div>
                      )}
                    </div>
                  )}
                </div>
              )}
            </div>
          </div>
        );
      })}

      {results.total > results.items.length && (
        <div className="text-center py-6">
          <button className="px-6 py-3 bg-legal-blue text-white rounded-xl font-medium hover:bg-legal-blue/90 transition-colors flex items-center gap-2 mx-auto">
            {isAr ? 'تحميل المزيد' : 'Charger plus'}
            <ChevronRight size={16} />
          </button>
        </div>
      )}
    </div>
  );
};

export default SearchResults;
