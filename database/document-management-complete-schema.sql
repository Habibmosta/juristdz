...
CREATE INDEX IF NOT EXISTS idx_document_comments_is_deleted ON document_comments(is_deleted) WHERE is_deleted = FALSE;
...
BEGIN
    IF NEW.is_current = TRUE THEN
        -- Set all other versions of this document to not current
...
-- Trigger for folder depth validation
...
CREATE POLICY "Users can view workflow signers for workflows they can see" ON workflow_signers
    FOR SELECT USING (
        auth.uid() = user_id OR
        EXISTS (
...
CREATE OR REPLACE VIEW document_permissions_summary AS
SELECT 
    d.id as document_id,
