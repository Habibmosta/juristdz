-- ═══════════════════════════════════════════════════════════════════════════
-- NETTOYAGE ET CRÉATION TABLES FACTURATION
-- ═══════════════════════════════════════════════════════════════════════════

-- 1. NETTOYAGE: Supprimer les anciennes versions si elles existent
DROP TRIGGER IF EXISTS trigger_update_invoice_payment ON public.payments;
...
DROP FUNCTION IF EXISTS create_invoice_from_time_entries(UUID, UUID, INTEGER);
...
CREATE INDEX idx_time_entries_is_invoiced ON public.time_entries(is_invoiced) WHERE is_invoiced = false;
...
-- 8. Politiques RLS pour invoices
...
-- 9. Politiques RLS pour invoice_items
...
-- 10. Politiques RLS pour payments
...
-- 11. Politiques RLS pour time_entries
...
-- 12. Fonction pour mettre à jour les totaux de facture
...
-- 13. Fonction pour mettre à jour les paiements
...
BEGIN
  ...
  UPDATE invoices
  SET 
    paid_amount = (SELECT COALESCE(SUM(amount), 0) FROM payments WHERE invoice_id = COALESCE(NEW.invoice_id, OLD.invoice_id)),
    updated_at = NOW()
  WHERE id = COALESCE(NEW.invoice_id, OLD.invoice_id);
...
-- 14. Fonction pour générer un numéro de facture
...
-- 15. Fonction pour créer une facture depuis les heures facturables
...
SELECT 
  ...
  (SELECT COUNT(*) FROM public.time_entries) as total_time_entries;
