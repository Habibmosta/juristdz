import { Pool } from 'pg';
import { BillingService } from '../services/billingService.js';
import {
  CalculateFeeRequest,
  CreateInvoiceRequest,
  LegalProfession,
  CalculationType,
  BillingStatus,
  InvoiceStatus,
  PaymentStatus,
  CaseComplexity,
  CourtLevel,
  UrgencyLevel,
  PropertyType,
  ExecutionType
} from '../types/billing.js';

// Mock the database pool
const mockDb = {
  query: jest.fn(),
  connect: jest.fn()
} as unknown as Pool;

// Mock logger
jest.mock('../utils/logger.js', () => ({
  logger: {
    info: jest.fn(),
    error: jest.fn(),
    warn: jest.fn(),
    debug: jest.fn()
  }
}));

describe('BillingService', () => {
  let billingService: BillingService;

  beforeEach(() => {
    billingService = new BillingService(mockDb);
    jest.clearAllMocks();
  });

  describe('calculateFees', () => {
    it('should calculate lawyer fees correctly', async () => {
      // Mock database responses
      (mockDb.query as jest.Mock)
        .mockResolvedValueOnce({
          // Mock calculate_lawyer_fees function result
          rows: [{
            base_fee: 15000,
            complexity_multiplier: 1.2,
            urgency_multiplier: 1.0,
            court_multiplier: 1.0,
            total_fee: 18000
          }]
        })
        .mockResolvedValueOnce({
          // Mock calculate_taxes function result
          rows: [{
            tax_type: 'vat',
            tax_rate: 0.19,
            tax_amount: 3420
          }]
        })
        .mockResolvedValueOnce({
          // Mock case query for lawyer_id
          rows: [{ lawyer_id: 'lawyer-123' }]
        })
        .mockResolvedValueOnce({
          // Mock save calculation
          rows: [{ id: 'calc-123' }]
        });

      const request: CalculateFeeRequest = {
        caseId: 'case-123',
        clientId: 'client-123',
        profession: LegalProfession.AVOCAT,
        calculationType: CalculationType.COURT_SCALE,
        parameters: {
          caseType: 'civil_litigation',
          courtLevel: CourtLevel.TRIBUNAL_PREMIERE_INSTANCE,
          caseComplexity: CaseComplexity.MODERATE,
          hours: 10,
          urgency: UrgencyLevel.NORMAL
        },
        applyTaxes: true,
        saveCalculation: true
      };

      const result = await billingService.calculateFees(request);

      expect(result).toMatchObject({
        calculationId: 'calc-123',
        totalAmount: expect.any(Number),
        taxAmount: expect.any(Number),
        finalAmount: expect.any(Number),
        currency: 'DZD'
      });

      expect(result.breakdown).toMatchObject({
        baseFee: 15000,
        complexityMultiplier: 1.2,
        urgencyMultiplier: 1.0,
        courtLevelMultiplier: 1.0
      });
    });

    it('should calculate notary fees correctly', async () => {
      (mockDb.query as jest.Mock)
        .mockResolvedValueOnce({
          // Mock calculate_notary_fees function result
          rows: [{
            base_fee: 50000,
            percentage_fee: 75000,
            document_fee: 0,
            total_fee: 75000
          }]
        });

      const request: CalculateFeeRequest = {
        caseId: 'case-123',
        clientId: 'client-123',
        profession: LegalProfession.NOTAIRE,
        calculationType: CalculationType.NOTARIAL_SCALE,
        parameters: {
          documentType: 'property_sale',
          propertyValue: 5000000,
          propertyType: PropertyType.RESIDENTIAL,
          documentCount: 1
        },
        applyTaxes: false,
        saveCalculation: false
      };

      const result = await billingService.calculateFees(request);

      expect(result.breakdown.baseFee).toBe(75000);
      expect(result.totalAmount).toBe(75000);
      expect(result.calculationId).toBeUndefined();
    });

    it('should calculate bailiff fees correctly', async () => {
      (mockDb.query as jest.Mock)
        .mockResolvedValueOnce({
          // Mock calculate_bailiff_fees function result
          rows: [{
            base_fee: 8000,
            distance_fee: 1000,
            urgency_multiplier: 1.5,
            total_fee: 13500
          }]
        });

      const request: CalculateFeeRequest = {
        caseId: 'case-123',
        clientId: 'client-123',
        profession: LegalProfession.HUISSIER,
        calculationType: CalculationType.BAILIFF_SCALE,
        parameters: {
          executionType: ExecutionType.SEIZURE,
          executionAmount: 100000,
          distance: 30,
          urgency: UrgencyLevel.URGENT
        },
        applyTaxes: false,
        saveCalculation: false
      };

      const result = await billingService.calculateFees(request);

      expect(result.breakdown.baseFee).toBe(9000); // base_fee + distance_fee
      expect(result.breakdown.urgencyMultiplier).toBe(1.5);
    });

    it('should handle additional fees and expenses', async () => {
      (mockDb.query as jest.Mock)
        .mockResolvedValueOnce({
          rows: [{
            base_fee: 10000,
            complexity_multiplier: 1.0,
            urgency_multiplier: 1.0,
            court_multiplier: 1.0,
            total_fee: 10000
          }]
        });

      const request: CalculateFeeRequest = {
        caseId: 'case-123',
        clientId: 'client-123',
        profession: LegalProfession.AVOCAT,
        calculationType: CalculationType.HOURLY_RATE,
        parameters: {
          hours: 5,
          travelExpenses: 2000,
          expertFees: 5000,
          courtFees: 1000,
          discountPercentage: 10
        },
        applyTaxes: false,
        saveCalculation: false
      };

      const result = await billingService.calculateFees(request);

      expect(result.breakdown.additionalFees).toHaveLength(3); // travel, expert, court
      expect(result.breakdown.additionalFees.some(fee => fee.type === 'travel')).toBe(true);
      expect(result.breakdown.additionalFees.some(fee => fee.type === 'expert')).toBe(true);
      expect(result.breakdown.additionalFees.some(fee => fee.type === 'court')).toBe(true);
    });

    it('should validate required fields', async () => {
      const invalidRequest = {
        caseId: '',
        clientId: 'client-123',
        profession: LegalProfession.AVOCAT,
        calculationType: CalculationType.HOURLY_RATE,
        parameters: {}
      } as CalculateFeeRequest;

      await expect(billingService.calculateFees(invalidRequest))
        .rejects.toThrow('Case ID is required');
    });

    it('should handle database errors gracefully', async () => {
      (mockDb.query as jest.Mock).mockRejectedValueOnce(new Error('Database connection failed'));

      const request: CalculateFeeRequest = {
        caseId: 'case-123',
        clientId: 'client-123',
        profession: LegalProfession.AVOCAT,
        calculationType: CalculationType.HOURLY_RATE,
        parameters: { hours: 5 },
        applyTaxes: false,
        saveCalculation: false
      };

      await expect(billingService.calculateFees(request))
        .rejects.toThrow('Database connection failed');
    });
  });

  describe('createInvoice', () => {
    it('should create invoice successfully', async () => {
      const mockCalculation = {
        id: 'calc-123',
        caseId: 'case-123',
        clientId: 'client-123',
        lawyerId: 'lawyer-123',
        totalAmount: 18000,
        taxAmount: 3420,
        finalAmount: 21420
      };

      (mockDb.query as jest.Mock)
        .mockResolvedValueOnce({
          // Mock getBillingCalculation
          rows: [{
            id: 'calc-123',
            case_id: 'case-123',
            client_id: 'client-123',
            lawyer_id: 'lawyer-123',
            profession: 'avocat',
            calculation_type: 'court_scale',
            base_amount: 15000,
            total_amount: 18000,
            tax_amount: 3420,
            final_amount: 21420,
            currency: 'DZD',
            calculation_date: new Date(),
            parameters: {},
            breakdown: {},
            status: 'calculated',
            created_at: new Date(),
            updated_at: new Date()
          }]
        })
        .mockResolvedValueOnce({
          // Mock generate_invoice_number
          rows: [{ invoice_number: 'FAC-2024-LAW123-0001' }]
        })
        .mockResolvedValueOnce({
          // Mock create invoice
          rows: [{
            id: 'invoice-123',
            invoice_number: 'FAC-2024-LAW123-0001',
            billing_calculation_id: 'calc-123',
            case_id: 'case-123',
            client_id: 'client-123',
            lawyer_id: 'lawyer-123',
            issue_date: new Date(),
            due_date: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000),
            payment_terms: 30,
            subtotal: 18000,
            tax_amount: 3420,
            total_amount: 21420,
            paid_amount: 0,
            remaining_amount: 21420,
            status: 'draft',
            payment_status: 'unpaid',
            created_at: new Date(),
            updated_at: new Date()
          }]
        })
        .mockResolvedValueOnce({
          // Mock update billing calculation status
          rows: []
        });

      const request: CreateInvoiceRequest = {
        billingCalculationId: 'calc-123',
        paymentTerms: 30,
        notes: 'Test invoice',
        sendToClient: false
      };

      const result = await billingService.createInvoice(request);

      expect(result).toMatchObject({
        id: 'invoice-123',
        invoiceNumber: 'FAC-2024-LAW123-0001',
        billingCalculationId: 'calc-123',
        status: InvoiceStatus.DRAFT,
        paymentStatus: PaymentStatus.UNPAID
      });
    });

    it('should handle missing billing calculation', async () => {
      (mockDb.query as jest.Mock).mockResolvedValueOnce({
        rows: [] // No billing calculation found
      });

      const request: CreateInvoiceRequest = {
        billingCalculationId: 'nonexistent-calc',
        paymentTerms: 30
      };

      await expect(billingService.createInvoice(request))
        .rejects.toThrow('Billing calculation not found');
    });
  });

  describe('getBillingCalculation', () => {
    it('should return billing calculation when found', async () => {
      (mockDb.query as jest.Mock).mockResolvedValueOnce({
        rows: [{
          id: 'calc-123',
          case_id: 'case-123',
          client_id: 'client-123',
          lawyer_id: 'lawyer-123',
          profession: 'avocat',
          calculation_type: 'court_scale',
          base_amount: 15000,
          total_amount: 18000,
          tax_amount: 3420,
          final_amount: 21420,
          currency: 'DZD',
          calculation_date: new Date(),
          parameters: {},
          breakdown: {},
          status: 'calculated',
          created_at: new Date(),
          updated_at: new Date()
        }]
      });

      const result = await billingService.getBillingCalculation('calc-123');

      expect(result).toMatchObject({
        id: 'calc-123',
        caseId: 'case-123',
        clientId: 'client-123',
        lawyerId: 'lawyer-123',
        profession: LegalProfession.AVOCAT,
        calculationType: CalculationType.COURT_SCALE
      });
    });

    it('should return null when not found', async () => {
      (mockDb.query as jest.Mock).mockResolvedValueOnce({
        rows: []
      });

      const result = await billingService.getBillingCalculation('nonexistent');

      expect(result).toBeNull();
    });
  });

  describe('searchBillingCalculations', () => {
    it('should search with basic criteria', async () => {
      const mockCalculations = [{
        id: 'calc-123',
        case_id: 'case-123',
        client_id: 'client-123',
        lawyer_id: 'lawyer-123',
        profession: 'avocat',
        calculation_type: 'court_scale',
        base_amount: 15000,
        total_amount: 18000,
        tax_amount: 3420,
        final_amount: 21420,
        currency: 'DZD',
        calculation_date: new Date(),
        parameters: {},
        breakdown: {},
        status: 'calculated',
        created_at: new Date(),
        updated_at: new Date()
      }];

      (mockDb.query as jest.Mock)
        .mockResolvedValueOnce({
          rows: mockCalculations
        })
        .mockResolvedValueOnce({
          rows: [{ count: '1' }]
        });

      const criteria = {
        profession: LegalProfession.AVOCAT,
        status: BillingStatus.CALCULATED,
        limit: 10,
        offset: 0
      };

      const result = await billingService.searchBillingCalculations(criteria);

      expect(result.calculations).toHaveLength(1);
      expect(result.totalCount).toBe(1);
      expect(result.searchTime).toBeGreaterThan(0);
    });

    it('should handle date range filtering', async () => {
      (mockDb.query as jest.Mock)
        .mockResolvedValueOnce({ rows: [] })
        .mockResolvedValueOnce({ rows: [{ count: '0' }] });

      const criteria = {
        dateRange: {
          from: new Date('2024-01-01'),
          to: new Date('2024-12-31')
        }
      };

      const result = await billingService.searchBillingCalculations(criteria);

      expect(result.calculations).toHaveLength(0);
      expect(result.totalCount).toBe(0);
    });

    it('should handle amount range filtering', async () => {
      (mockDb.query as jest.Mock)
        .mockResolvedValueOnce({ rows: [] })
        .mockResolvedValueOnce({ rows: [{ count: '0' }] });

      const criteria = {
        amountRange: {
          min: 10000,
          max: 50000
        }
      };

      const result = await billingService.searchBillingCalculations(criteria);

      expect(result.calculations).toHaveLength(0);
      expect(result.totalCount).toBe(0);
    });
  });

  describe('getBillingStatistics', () => {
    it('should return comprehensive statistics', async () => {
      (mockDb.query as jest.Mock)
        .mockResolvedValueOnce({
          // Basic statistics
          rows: [{
            total_calculations: '10',
            total_amount: '180000',
            average_amount: '18000'
          }]
        })
        .mockResolvedValueOnce({
          // By profession
          rows: [
            { profession: 'avocat', count: '8' },
            { profession: 'notaire', count: '2' }
          ]
        })
        .mockResolvedValueOnce({
          // By type
          rows: [
            { calculation_type: 'court_scale', count: '6' },
            { calculation_type: 'hourly_rate', count: '4' }
          ]
        })
        .mockResolvedValueOnce({
          // By status
          rows: [
            { status: 'calculated', count: '7' },
            { status: 'invoiced', count: '3' }
          ]
        });

      const result = await billingService.getBillingStatistics();

      expect(result).toMatchObject({
        totalCalculations: 10,
        totalAmount: 180000,
        averageAmount: 18000,
        calculationsByProfession: {
          [LegalProfession.AVOCAT]: 8,
          [LegalProfession.NOTAIRE]: 2
        },
        calculationsByType: {
          [CalculationType.COURT_SCALE]: 6,
          [CalculationType.HOURLY_RATE]: 4
        },
        calculationsByStatus: {
          [BillingStatus.CALCULATED]: 7,
          [BillingStatus.INVOICED]: 3
        }
      });
    });

    it('should filter by lawyer when provided', async () => {
      (mockDb.query as jest.Mock)
        .mockResolvedValueOnce({
          rows: [{
            total_calculations: '5',
            total_amount: '90000',
            average_amount: '18000'
          }]
        })
        .mockResolvedValueOnce({ rows: [] })
        .mockResolvedValueOnce({ rows: [] })
        .mockResolvedValueOnce({ rows: [] });

      const result = await billingService.getBillingStatistics('lawyer-123');

      expect(result.totalCalculations).toBe(5);
      expect(result.totalAmount).toBe(90000);
    });
  });

  describe('updateFeeSchedules', () => {
    it('should update fee schedules successfully', async () => {
      const mockClient = {
        query: jest.fn(),
        release: jest.fn()
      };

      (mockDb.connect as jest.Mock).mockResolvedValueOnce(mockClient);
      
      mockClient.query
        .mockResolvedValueOnce({ rows: [] }) // BEGIN
        .mockResolvedValueOnce({ rows: [] }) // INSERT/UPDATE schedule
        .mockResolvedValueOnce({ rows: [] }) // INSERT log
        .mockResolvedValueOnce({ rows: [] }); // COMMIT

      const request = {
        profession: LegalProfession.AVOCAT,
        schedules: [{
          category: 'Consultation juridique',
          minimumFee: 2500,
          unitType: 'per_hour',
          legalReference: 'Updated reference 2024'
        }],
        effectiveDate: new Date('2024-01-01'),
        reason: 'Annual fee update'
      };

      await expect(billingService.updateFeeSchedules(request))
        .resolves.not.toThrow();

      expect(mockClient.query).toHaveBeenCalledWith('BEGIN');
      expect(mockClient.query).toHaveBeenCalledWith('COMMIT');
      expect(mockClient.release).toHaveBeenCalled();
    });

    it('should rollback on error', async () => {
      const mockClient = {
        query: jest.fn(),
        release: jest.fn()
      };

      (mockDb.connect as jest.Mock).mockResolvedValueOnce(mockClient);
      
      mockClient.query
        .mockResolvedValueOnce({ rows: [] }) // BEGIN
        .mockRejectedValueOnce(new Error('Database error')); // INSERT fails

      const request = {
        profession: LegalProfession.AVOCAT,
        schedules: [{}],
        effectiveDate: new Date(),
        reason: 'Test update'
      };

      await expect(billingService.updateFeeSchedules(request))
        .rejects.toThrow('Database error');

      expect(mockClient.query).toHaveBeenCalledWith('ROLLBACK');
      expect(mockClient.release).toHaveBeenCalled();
    });
  });

  describe('Edge Cases and Error Handling', () => {
    it('should handle unsupported profession in fee calculation', async () => {
      const request: CalculateFeeRequest = {
        caseId: 'case-123',
        clientId: 'client-123',
        profession: 'unsupported_profession' as LegalProfession,
        calculationType: CalculationType.HOURLY_RATE,
        parameters: {},
        applyTaxes: false,
        saveCalculation: false
      };

      await expect(billingService.calculateFees(request))
        .rejects.toThrow('Unsupported profession');
    });

    it('should handle empty fee schedule results', async () => {
      (mockDb.query as jest.Mock).mockResolvedValueOnce({
        rows: [] // No fee schedule found
      });

      const request: CalculateFeeRequest = {
        caseId: 'case-123',
        clientId: 'client-123',
        profession: LegalProfession.AVOCAT,
        calculationType: CalculationType.COURT_SCALE,
        parameters: {
          caseType: 'unknown_case_type'
        },
        applyTaxes: false,
        saveCalculation: false
      };

      await expect(billingService.calculateFees(request))
        .rejects.toThrow('No fee schedule found');
    });

    it('should handle missing case for save calculation', async () => {
      (mockDb.query as jest.Mock)
        .mockResolvedValueOnce({
          // Mock fee calculation
          rows: [{
            base_fee: 10000,
            complexity_multiplier: 1.0,
            urgency_multiplier: 1.0,
            court_multiplier: 1.0,
            total_fee: 10000
          }]
        })
        .mockResolvedValueOnce({
          // Mock case query - no case found
          rows: []
        });

      const request: CalculateFeeRequest = {
        caseId: 'nonexistent-case',
        clientId: 'client-123',
        profession: LegalProfession.AVOCAT,
        calculationType: CalculationType.HOURLY_RATE,
        parameters: { hours: 5 },
        applyTaxes: false,
        saveCalculation: true
      };

      await expect(billingService.calculateFees(request))
        .rejects.toThrow('Case not found');
    });
  });
});